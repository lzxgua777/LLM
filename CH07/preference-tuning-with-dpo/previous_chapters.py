...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        return x
# 定义GPT模型类，继承自PyTorch的nn.Module
class GPTModel(nn.Module):
    def __init__(self, cfg):
        super().__init__()  # 调用父类的构造函数
        # 定义词嵌入层，将词汇表大小映射到嵌入维度
        self.tok_emb = nn.Embedding(cfg["vocab_size"], cfg["emb_dim"])
        # 定义位置嵌入层，将上下文长度映射到嵌入维度
        self.pos_emb = nn.Embedding(cfg["context_length"], cfg["emb_dim"])
        # 定义嵌入层的dropout
        self.drop_emb = nn.Dropout(cfg["drop_rate"])

        # 定义Transformer块序列，数量由配置中的层数决定
        self.trf_blocks = nn.Sequential(
            *[TransformerBlock(cfg) for _ in range(cfg["n_layers"])])

        # 定义最终的层归一化
        self.final_norm = LayerNorm(cfg["emb_dim"])
        # 定义输出层，将嵌入维度映射到词汇表大小，不使用偏置
    ...
    def forward(self, in_idx):
        ...
        # 合并词嵌入和位置嵌入
        x = tok_embeds + pos_embeds  # 形状：[批次大小, 序列长度, 嵌入维度]
        # 应用嵌入层的dropout
        x = self.drop_emb(x)
        # 通过Transformer块序列
        x = self.trf_blocks(x)
...
def generate_text_simple(model, idx, max_new_tokens, context_size):
    ...
    for _ in range(max_new_tokens):
        ...
        # 将采样的索引追加到正在运行的序列中
        idx = torch.cat((idx, idx_next), dim=1)  # (批次大小, 序列长度+1)
...
# 定义一个生成文本的函数，该函数接受模型、索引、最大新令牌数、上下文大小、温度参数、最高k值采样和结束序列标记
def generate(model, idx, max_new_tokens, context_size, temperature=0.0, top_k=None, eos_id=None):
    ...
    for _ in range(max_new_tokens):
        ...
        if temperature > 0.0:
            ...
            # 应用softmax获取概率分布
            probs = torch.softmax(logits, dim=-1)  # (批次大小, 词汇表大小)
...
def plot_losses(epochs_seen, tokens_seen, train_losses, val_losses, label="loss"):
    ...
    # 绘制训练和验证损失随epoch的变化
    ax1.plot(epochs_seen, train_losses, label=f"Training {label}")  # 绘制训练损失
    ax1.plot(epochs_seen, val_losses, linestyle="-.", label=f"Validation {label}")  # 绘制验证损失
    ...
    plt.savefig(f"{label}-plot.pdf")  # 保存图形
    plt.show()  # 显示图形